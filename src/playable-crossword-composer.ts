...
function installPlayableComposer(): void {
  ...
  const exportCurrentCrossword = (): void => { if (!composedCrossword) return; downloadCrossword(composedCrossword); status.textContent = `${composedCrossword.name} exporté en JSON.`; };
