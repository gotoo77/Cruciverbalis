...
function decideProposal(
  ...
  if (proposal.status !== 'pending') {
    throw new Error(`editorial proposal ${proposal.id} is already ${proposal.status}`);
  }
  ...
  return {
    ...
    decisionNote: options.decisionNote?.trim() || undefined,
  };
}

/** Une proposition n'acquiert d'autorité qu'après cette action humaine explicite. */
...
export function serializeEditorialProposal(proposal: EditorialProposal): string {
