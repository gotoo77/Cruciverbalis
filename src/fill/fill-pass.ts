...
/**
 * V0 reste volontairement prudent : seuls les ponts déjà contraints par la
 * grille thématique sont proposés au remplissage.
 */
    ...
    .filter(
      (slot, index, all) =>
        !all
          ...
          .some(
            (other) =>
              other.direction === slot.direction &&
              other.row === slot.row &&
              other.col === slot.col &&
              other.length < slot.length,
          ),
    );
}
...
function matches(pattern: string, word: string): boolean {
  return (
    ...
    [...pattern].every((letter, index) => letter === '?' || letter === word[index])
  );
}
...
function policyFor(options: FillPassOptions): LexicalQualityPolicy {
  return {
    ...DEFAULT_LEXICAL_QUALITY_POLICY,
    minLength: options.minLength ?? DEFAULT_LEXICAL_QUALITY_POLICY.minLength,
    ...options.lexicalPolicy,
  };
}

function scoredCandidatesForSlot(
  grid: DomainGrid,
  slot: FillSlot,
  dictionary: readonly (string | LexicalCandidate)[],
  used: ReadonlySet<string>,
  policy: LexicalQualityPolicy,
): ScoredLexicalCandidate[] {
  const pattern = currentPattern(grid, slot);
  return normalizeLexicon(dictionary)
    .map((candidate) => scoreLexicalCandidate(candidate, policy))
    .filter((candidate): candidate is ScoredLexicalCandidate => Boolean(candidate))
    .filter(({ word }) => !used.has(word) && matches(pattern, word))
    .sort((a, b) => b.score - a.score || a.word.localeCompare(b.word));
}

/**
 * Contrat public historique : retourne uniquement les mots candidats.
 * Le score lexical reste un détail du moteur de recherche.
 */
...
export function fillSeedGrid(
  ...
  const visit = (
    grid: DomainGrid,
    remaining: readonly FillSlot[],
    filled: readonly Placement[],
    used: ReadonlySet<string>,
    lexicalScore: number,
  ): void => {
    ...
    if (
      filled.length > bestFilled.length ||
      (filled.length === bestFilled.length && lexicalScore > bestScore)
    ) {
      bestGrid = grid;
      bestFilled = [...filled];
    ...
    const ranked = remaining
      .map((slot) => ({
        slot,
        candidates: scoredCandidatesForSlot(grid, slot, dictionary, used, policy),
      }))
      .sort(
        (a, b) =>
          a.candidates.length - b.candidates.length ||
          b.slot.anchors - a.slot.anchors ||
    ...
    for (const candidate of chosen.candidates) {
      const placement: Placement = {
        entry: { answer: candidate.word },
        start: { row: chosen.slot.row, col: chosen.slot.col },
  ...
  const filledIds = new Set(
    bestFilled.map(
      (placement) =>
        `${placement.direction}:${placement.start.row},${placement.start.col}:${normalizeAnswer(placement.entry.answer).length}`,
    ),
  );
  ...
  return {
    ...
    slots,
    filled: bestFilled,
    unfilled: slots.filter((slot) => !filledIds.has(slot.id)),
    stats: {
      nodesExplored,
      backtracks,
      slotsDetected: slots.length,
      slotsFilled: bestFilled.length,
      lexicalScore: bestFilled.length === 0 ? 0 : bestScore,
    },
    truncated,
  };
}
